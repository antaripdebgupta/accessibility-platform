        ...
        <div v-else-if="isCrawling" class="space-y-4">
          ...
          <div class="h-2 w-full overflow-hidden rounded-full bg-gray-200">
            <div
              ...
              :style="{ width: `${crawlProgress.percent || 10}%` }"
            ></div>
          </div>
          <!-- Live discovered pages counter -->
          <div
            v-if="crawlProgress.pagesFound > 0"
            class="flex items-center space-x-2 text-sm text-indigo-600"
          >
            <svg
              class="h-4 w-4"
              fill="none"
            ...
            <span class="font-medium"
              >{{ crawlProgress.pagesFound }} pages discovered</span
            >
          </div>
          <!-- Last discovered URL -->
          <div
            v-if="crawlProgress.lastUrl"
            class="text-xs text-gray-500 truncate"
          >
            Latest: {{ truncateUrl(crawlProgress.lastUrl) }}
          </div>
        </div>
...
// SSE-driven crawl progress
const crawlProgress = computed(() => {
  if (!currentTaskId.value) {
    return {
      percent: 10,
      message: 'Exploring website...',
      pagesFound: 0,
      lastUrl: null,
    }
  }
  ...
  return {
    ...
    message: progress?.message || 'Exploring website...',
    pagesFound: progress?.pagesFound || 0,
